...
- **Interaction feedback (desktop convention): press > hover.** Every control has an instant **pressed** (`active:`) state one step deeper than its hover — native buttons acknowledge the press, not the hover; hover-without-press is backwards. Hover exists but stays **subtle** (one quiet step: `hover:bg-muted` on gray controls, a small alpha bump on tinted ones, `hover:brightness-105` on the ActionButton gradient); a loud hover is a consumer-web tell. List rows (menu items, tree rows, options) hover-highlight and select on click — no separate pressed step. All of it is instant (no-transition rule).
