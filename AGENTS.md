...
- **Density is a knob: `--density`** (default 1). It scales `--spacing` (and everything derived — gaps, `--pad-box`, control heights `--spacing-control`), so the whole layout breathes from one number while type/icons/borders stay fixed. Set on `:root` globally or scope to a subtree (`.density-compact`/`-cozy`/`-comfortable`, or `style="--density:N"`). ~1 = instrument (22px controls) · 1.5 = comfortable (~33px). Because of this, ALWAYS express spacing as `--spacing` multiples (`gap-*`, `p-*`, `pad-box`) and control heights as `h-control`/`h-control-sm` — never raw px — or it won't respond to density.
