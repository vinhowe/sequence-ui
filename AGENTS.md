...
- **Density is a knob: `--density`** (default 1) — and it's an **air** knob, not a zoom. It scales *only the whitespace*: the `stack-*` gaps (field/group/section) and `pad-box`. It does NOT scale `--spacing`, control heights, control-internal padding, type, icons, or borders — those stay fixed, so controls are the same crisp instrument size at any density; only the room around them changes. (Scaling everything incl. type is just `cmd +`; scaling space but freezing type over-pads controls — scaling only air avoids both.) Set on `:root` globally or scope to a subtree (`.density-compact`/`-cozy`/`-comfortable`, or `style="--density:N"`). `1` = instrument · `1.5` = cozy · `2` = comfortable (roomy). Use `stack-*`/`pad-box` for anything that should breathe; use `h-control`/`h-control-sm` + `--spacing` multiples (never raw px) for the controls, which keeps them fixed and consistent. Note `stack-tight` (label→field) is intentionally fixed so proximity grouping sharpens as things spread.
...
- Icons come from Lucide. Size icon glyphs in **fixed px** (10 / 11 / 13), never `--spacing`-multiples (`h-3.5`). They stay fixed at every density (density scales only whitespace), which is what you want — crisp integer-px glyphs, no fractional-px softness.
