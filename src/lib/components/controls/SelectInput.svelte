...
<div class={twMerge('relative stack-tight', wrapperClass)} bind:this={wrapperEl}>
	...
	<div class="flex gap-1.5 items-center">
		<div class="relative flex-1">
			...
			<div
				class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-1 text-subtle-foreground"
			>
			...
			{#if isOpen}
				...
				<Portal target="body">
					...
					class="fixed z-[9999] border border-border bg-popover text-popover-foreground max-h-120 overflow-auto rounded-none shadow-lg"
						style={menuStyle}
					>
						{#each menuItems as item, i (item.id)}
							...
							{:else}
								...
								<li
									...
									class:bg-muted={i === highlightedIndex && !isDisabled}
