...
- **Density knob.** `--density` (default 1) scales `--spacing` and everything derived (gaps, `--pad-box`, control heights) AND the type scale — one number, whole layout breathes proportionally; only borders (1px) and icon glyphs stay fixed. Global on `:root` or scoped (`.density-cozy`/`-comfortable`, or `style="--density:N"`). So always use `--spacing` multiples + `h-control`, never raw px, or it won't scale.
