...
- **Density knob.** `--density` (default 1) scales `--spacing` and everything derived (gaps, `--pad-box`, control heights) — one number, whole layout breathes; type/icons/borders stay fixed. Global on `:root` or scoped (`.density-cozy`/`-comfortable`, or `style="--density:N"`). So always use `--spacing` multiples + `h-control`, never raw px, or it won't scale.
