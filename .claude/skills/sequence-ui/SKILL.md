...
- **Container-owned spacing.** Base grid `--spacing: 0.25rem` (4px = 1U). Components ship margin-free (no `mt-*`/`mb-*`). Parents stack children with `stack-tight` / `stack-field` / `stack-group` (all 4px) and `stack-section` (16px between sections). A control's label→field gap is `stack-tight` on the control root. Content boxes pad with the `pad-box` utility (not hand-picked `p-*`); running prose uses the `prose` class (owns its own rhythm — the one sanctioned child margin). Run **tight** — SaaS-comfortable spacing is too loose here. **A "section" means an `h2`-led page region (specimen/doc pages).** Most product pages (settings, inspectors, control surfaces) are just a **stack of titled panels**: the `Panel` title IS the heading — no `h2`s, no `stack-section`, panels stack at `gap-1` (4px). Never pair an `h2` with a single Panel repeating the same word; never put 16px between plain panels.
- **Size fields to content.** Numeric/unit fields get a content-appropriate width (`class="w-24"` etc.); only free text, paths, and URLs fill the column. Numeric picks: `Slider` = bounded continuous (position informs); `ScrubInput` = any fine-tunable numeric (drag+type, step/precision/unit, floats) — the default; `NumberInput` = plain counts. `hasDefaultValue` means "value currently equals its default" (disables reset). Destructive actions confirm **inline** (two-step arm→confirm on the button + amber `Note`) — no modals exist, don't hand-roll one.
...
- **Press > hover.** Every control has an instant pressed (`active:`) state one step deeper than its hover; hover stays subtle (one quiet step — loud hover is a consumer-web tell). List rows just hover-highlight. Never hover-without-press.
...
5. **Project-header motif (optional, on-brand).** Use the **`AppBar`** component: `<AppBar title="…"><ThemeToggle integrated /></AppBar>` — a full-width, brand-hued identity bar. Pass `integrated` to `ThemeToggle` so it docks as a full-height bar segment (dividers + active chip read the same `--bar-*` tokens). If hand-rolling, its height is app *chrome*, not grid: fixed integer `h-[var(--bar-height)]` (20px), `pl-1.5`, and **never `py-*`** (at ~20px, padding rounds unevenly and drifts the toggle/text off-center). Colors come from the `--bar-*` tokens: `bg-bar text-bar-foreground border-t border-t-transparent border-b border-b-bar-border`; project name `font-mono uppercase tracking-wider text-xs`.
